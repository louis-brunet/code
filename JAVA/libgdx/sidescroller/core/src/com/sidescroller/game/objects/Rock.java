...
public class Rock extends AbstractGameObject {
	...
	private int length; // Number of middle parts
	...
	public void setLength(int length) {
		...
		// Update bounding bow for collision detection
	...
	public void render(SpriteBatch batch) {
		...
		batch.draw(reg.getTexture(), position.x + relX, 
				position.y + relY, origin.x, origin.y, 
				dimension.x/4, dimension.y, scale.x, scale.y,
		...
		reg = regEdge;
		batch.draw(reg.getTexture(), position.x + relX, 
				position.y + relY, origin.x + dimension.x/8, origin.y, 
				dimension.x/4, dimension.y, scale.x, scale.y,
				rotation, reg.getRegionX(), reg.getRegionY(),
